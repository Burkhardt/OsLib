...
- Current release line for `OsLibCore` is `3.11.3`.
- Coordinated patch-line refresh: package metadata, release notes, README links, and diagram markers stay aligned with the `RAIkeep` `3.11.3` release line.
- No public API changes from `3.11.0`; the `RaiFile.mkdir()` virtual dispatch, UTC timestamp handling, and async `RaiFile` APIs remain current.
- See [RELEASE_NOTES_3.11.3.md](RELEASE_NOTES_3.11.3.md) for details.
...
- Typical release command:
	- `git tag -a v3.11.3 -m "v3.11.3" && git push origin v3.11.3`
